...
async function aiPage() {
  ...
  return pageHeading('ai', `<a href="#/${settings.configured ? 'resumes' : 'settings'}" class="button button-primary button-small">${settings.configured ? '去简历编辑器润色' : '连接 DeepSeek'} ${icon('arrow')}</a>`) + `<div class="info-strip">${icon('ai')}<p>${settings.configured ? 'DeepSeek 已配置。' : 'DeepSeek 尚未配置。'} 在简历编辑器中选择一段真实经历，生成建议后对比、采纳。所有报告关联生成时的简历版本。</p></div><div class="ai-report-list">${reports.length ? reports.map(r => `<article class="panel report-card"><div class="panel-heading"><h2>${e(r.field_path === 'basics.summary' ? '个人简介润色' : r.field_path === 'skills' ? '专业技能润色' : '经历表达润色')} <span>V${r.resume_revision}</span></h2><span class="badge ${r.applied_at ? 'badge-green' : 'badge-slate'}">${r.applied_at ? '已采纳' : '待参考'}</span></div><div class="report-excerpt"><div><small>原文</small><p>${e(r.original.slice(0, 180))}</p></div><div><small>润色建议</small><p>${e(r.revised.slice(0, 240))}</p></div></div><div class="report-footer"><span>${e(r.model)} · ${time(r.created_at)}</span><button class="text-link" data-action="report-detail" data-id="${r.id}">查看完整对比 →</button></div></article>`).join('') : emptyState('还没有润色记录', '先创建简历，写下一段真实经历，再让 AI 帮你整理表达。', '<a class="button button-primary" href="#/resumes">去写简历</a>')}</div>`;
}
async function settingsPage() {
  if (!state.user) return loginEmpty('settings');
  const config = await api('/settings/ai');
  return pageHeading('settings') + `<div class="settings-layout"><section class="panel settings-panel"><div class="panel-heading"><h2>${icon('ai')} DeepSeek 接入</h2><span class="badge ${config.configured ? 'badge-green' : 'badge-slate'}">${config.configured ? '已配置' : '待配置'}</span></div><p class="settings-description">使用你的 DeepSeek API 账户。密钥由后端加密保存，不会回显到浏览器；只有点击润色时才调用。</p><form id="ai-settings-form"><label class="field"><span>API 密钥${config.source === 'personal' ? '（已保存，留空则不修改）' : ''}</span><input name="apiKey" type="password" autocomplete="new-password" maxlength="500" placeholder="在这里填写 DeepSeek API 密钥"></label><label class="field"><span>模型</span><select name="model">${config.models.map(model => `<option value="${model}" ${config.model === model ? 'selected' : ''}>${model === 'deepseek-flash' ? 'DeepSeek Flash · 日常润色' : 'DeepSeek V4 Pro · 深入优化'}</option>`).join('')}</select></label><p class="form-error" role="alert"></p><div class="button-row"><button class="button button-primary button-small" type="submit">保存配置</button>${config.source === 'personal' ? '<button class="button button-soft button-small" id="remove-ai-key" type="button">移除个人密钥</button>' : ''}<a href="https://platform.deepseek.com/api_keys" target="_blank" rel="noopener noreferrer" class="text-link">DeepSeek 官方平台 ↗</a></div><p class="field-note">当前来源：${config.source === 'personal' ? '个人配置' : config.source === 'server' ? '服务器环境配置' : '尚未配置'}。保存配置不会自动发起调用，也不代表已经验证密钥可用。</p></form></section><div><section class="panel settings-panel"><div class="panel-heading"><h2>我的账号</h2></div><div class="profile-summary"><span class="user-avatar">${e(state.user.name.slice(0, 1))}</span><div><strong>${e(state.user.name)}</strong><small>${e(state.user.email)}</small></div></div><button class="button button-soft button-small" data-action="logout">${icon('logout')} 退出登录</button></section><section class="panel settings-panel"><div class="panel-heading"><h2>数据与备份</h2></div><p class="settings-description">导出你的投递、待办与简历内容。导出文件不含密码或 API 密钥，请妥善保存个人资料。</p><button class="button button-soft button-small" data-action="export-data">${icon('download')} 导出我的数据</button><p class="field-note">本地数据保存在启动这个项目的电脑。邮箱暂仅用作账号，不提供邮件找回。</p></section></div></div><section class="panel source-panel"><div class="panel-heading"><h2>岗位数据说明</h2></div><p>本批 ${state.meta.jobs.count} 条，来源：${state.meta.companies.map(c => `${e(c.company)} ${c.count} 条`).join('、')}。采集于 ${date(state.meta.jobs.collected_at)}。</p><p>${e(state.meta.jobs.limitations)}</p><p>点击条目可查看企业官网或西北大学岗位页、2027 届依据和原始发布日期。当前不会自动刷新外部招聘状态。</p></section>`;
}
...
function wirePage() {
  ...
  aiForm?.addEventListener('submit', async event => { event.preventDefault(); const btn = aiForm.querySelector('[type=submit]'); btn.disabled = true; try { await api('/settings/ai', { method: 'PUT', data: Object.fromEntries(new FormData(aiForm)) }); aiForm.elements.apiKey.value = ''; toast('配置已保存；尚未发起 API 调用。'); await renderRoute(); } catch (err) { aiForm.querySelector('.form-error').textContent = err.message; btn.disabled = false; } });
  document.getElementById('remove-ai-key')?.addEventListener('click', async () => { if (!confirm('移除保存在本机的个人 DeepSeek 密钥？')) return; try { await api('/settings/ai', { method: 'PUT', data: { model: aiForm.elements.model.value, removeKey: true } }); await renderRoute(); toast('个人密钥已移除。'); } catch (err) { errorMessage(err); } });
