...
export function mountEditor(container, initial, onSaved = () => {}, options = {}) {
  ...
  async function showPolish() {
    if (local) { toast('先点击“存入我的简历”，再使用已连接的 DeepSeek 润色。'); return; }
    const [settings, applications] = await Promise.all([api('/settings/ai'), api('/applications')]);
    if (!settings.configured) {
      dialog('先连接 DeepSeek', '<p class="modal-description">在设置中填写你的 DeepSeek API 密钥。密钥保存在后端，页面不会回显；配置后即可润色真实经历。</p><a class="button button-primary" href="#/settings" id="go-settings">前往设置 →</a>', el => el.querySelector('#go-settings').addEventListener('click', () => el.close())); return;
    }
    const options = [];
    ...
    dialog('DeepSeek · 润色这一段', `<div id="polish-content"><p class="modal-description">围绕真实经历优化表达。不会自动发送姓名、手机、邮箱，也不会直接覆盖简历。</p><form id="polish-form"><label class="field"><span>选择模块</span><select name="field">${options.map(([value, label]) => `<option value="${e(value)}" ${value === preferred ? 'selected' : ''}>${e(label)}</option>`).join('')}</select></label><label class="field"><span>目标岗位（来自我的投递）</span><select name="jobId"><option value="">不指定岗位</option>${uniqueJobs.map(j => `<option value="${e(j.id)}">${e(j.company)} · ${e(j.title)}</option>`).join('')}</select></label><label class="field"><span>润色方向</span><select name="tone"><option value="professional">表达更专业、清晰</option><option value="concise">精简重复，突出重点</option><option value="star">围绕情境、行动、结果组织</option></select></label><div class="original-preview"><span>即将发送的原文</span><p id="polish-original">${e(get(preferred))}</p></div><label class="consent"><input type="checkbox" name="consent" required><span>同意将这段文字及所选岗位要求发送给 DeepSeek。若段落含个人敏感信息，请先删去。调用可能消耗你的 API 余额。</span></label><div class="modal-actions"><span class="muted">${e(settings.model)}</span><button class="button button-primary" type="submit">${icon('ai')} ${dirty ? '先保存，再润色' : '开始润色'}</button></div><p class="form-error" role="alert"></p></form></div>`, el => {
      ...
      form.addEventListener('submit', async event => {
        ...
        const btn = form.querySelector('button[type=submit]'); btn.disabled = true; btn.textContent = 'DeepSeek 正在润色…'; form.querySelector('.form-error').textContent = '';
        try {
          await save();
          ...
          el.querySelector('#polish-content').innerHTML = `<div class="polish-comparison"><section><h3>原文 · V${report.resume_revision}</h3><p>${e(report.original)}</p></section><section class="revised-text"><h3>润色建议</h3><p>${e(report.revised)}</p></section></div><div class="ai-reasons"><h3>为什么这样改</h3><ul>${report.changes.map(s => `<li>${e(s)}</li>`).join('')}</ul>${report.questions.length ? `<h3>建议你补充的事实</h3><ul>${report.questions.map(s => `<li>${e(s)}</li>`).join('')}</ul>` : ''}</div><p class="modal-description">请核对数字、经历与职责。采纳后生成新版本，原版本仍可恢复。</p><div class="modal-actions"><button class="button button-soft" id="copy-revised">复制建议</button><button class="button button-primary" id="apply-revised">采纳到简历 ${icon('check')}</button></div>`;
